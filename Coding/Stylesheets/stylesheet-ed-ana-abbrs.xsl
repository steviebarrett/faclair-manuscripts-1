		...
		<html>
			...
			<body>
				<table border="1px solid black">
							...
							<td>
								<xsl:choose>
									<xsl:when test="ancestor::tei:div//tei:handShift">
										...
										<xsl:text> (</xsl:text>
										<xsl:value-of
											select="preceding::tei:handShift[1]/@new or preceding::tei:div[1]/@resp"/>
										<xsl:text>)</xsl:text>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of
					...
					<tr><th>Tally</th></tr>
					<xsl:for-each select="//tei:g">
						<xsl:variable name="glyph" select="@ref"/>
						<xsl:if test="not(@ref = preceding::tei:g/@ref)">
							<tr>
								<td>
									<xsl:value-of select="key('abbrs', @ref)/tei:glyphName"/>
								</td>
								<td> 
									<xsl:value-of select="count(//tei:g[@ref = $glyph])"/>
								</td>
							</tr>
						</xsl:if>
					</xsl:for-each>
