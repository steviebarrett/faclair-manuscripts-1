...
<xsl:stylesheet xmlns:tei="http://www.tei-c.org/ns/1.0"
    ...
    <xsl:template name="nodes">
        <xsl:for-each select="//tei:TEI"
            ...
            <xsl:element name="node">
                ...
                <xsl:attribute name="label" select="concat(descendant::tei:msIdentifier/tei:repository, ' ', descendant::tei:msIdentifier/tei:idno)"/>
                <viz:size value="120"/>
    ...
    <xsl:template name="edges">
        <xsl:for-each select="//tei:w[@lemmaRef and not(@type = 'data')]"
            ...
            <xsl:variable name="ms" select="string(ancestor::tei:TEI/@xml:id)"/>
