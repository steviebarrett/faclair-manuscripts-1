...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template match="ns:w | ns:name | ns:g | ns:pc | ns:date | ns:num | ns:c | ns:gap | ns:seg[@type = 'fragment']">
        <xsl:copy>
            <xsl:attribute name="id">
                <xsl:variable name="id" select="generate-id(.)"/>
                <xsl:value-of select="concat(translate(string(ancestor::ns:TEI/@xml:id), 'DT', 'dt'), $id)"
                />
            </xsl:attribute>
            <xsl:apply-templates select="@* | node()"/>
        </xsl:copy>
