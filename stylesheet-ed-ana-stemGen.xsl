...
<xsl:stylesheet xmlns:tei="http://www.tei-c.org/ns/1.0"
	...
	<xsl:template name="contentRow">
		...
		<tr>
				...
				<xsl:choose>
					...
					<xsl:when test="count(key('hwData', @lemmaRef)/descendant::tei:iType) > 1">
						<xsl:variable name="stemCount"
							select="count(key('hwData', @lemmaRef)/descendant::tei:iType)"/>
			...
			<td>
				<xsl:choose>
					<xsl:when test="count(key('hwData', @lemmaRef)/descendant::tei:gen) = 1">
						<xsl:value-of select="key('hwData', @lemmaRef)/descendant::tei:gen"/>
					</xsl:when>
					<xsl:when test="count(key('hwData', @lemmaRef)/descendant::tei:gen) > 1">
						<xsl:variable name="genCount"
							select="count(key('hwData', @lemmaRef)/descendant::tei:gen)"/>
						<xsl:for-each select="key('hwData', @lemmaRef)/descendant::tei:gen">
							<xsl:choose>
								<xsl:when test="position() &lt; $genCount">
									<xsl:value-of select="self::*"/>
									<xsl:text> (</xsl:text>
									<xsl:value-of select="@xml:lang"/>
									<xsl:text>)</xsl:text>
